...
namespace Fhnw.Ecnf.RoutePlanner.RoutePlannerLib
{
	...
	public class RoutesFloydWarshall: Routes
	{
		...
		public override List<Link> FindShortestRouteBetween(string fromCity, string toCity,
										TransportModes mode)
		{
			List<City> cities2 = cities.FindCitiesBetween(fromCity, toCity);
			if (cities2 == null || cities2.Count < 1)
				return null;

			List<Link> links = FindAllLinks(cities2, mode);
			...
			City source = FindCity( fromCity, cities2 );
			City target = FindCity(toCity, cities2);
		...
		{
			if (ExecuteParallel) SetupParallel(cities, links);
			else SetupSerial(cities, links);
		}

		private void SetupSerial(List<City> cities, List<Link> links)
		{
        ...
        {
            if (ExecuteParallel) return InitializeWeightParallel(cities, links);
            return InitializeWeightSerial(cities, links);
        }

		private double[,] InitializeWeightSerial(List<City> cities, List<Link> links)
		{
